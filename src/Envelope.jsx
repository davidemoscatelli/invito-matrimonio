...
export default function Envelope({ onOpen, onPlayMusic }) {
  ...
  const handleOpen = () => {
    ...
    // Fai partire la musica esattamente al momento del clic!
    if (onPlayMusic) onPlayMusic();

    // Aspetta l'apertura (0.8s) + uscita lettera (1s) + tempo di lettura, poi zooma
    setTimeout(() => {
      setIsZooming(true);
      setTimeout(() => {
        onOpen();
  ...
  return (
    ...
    >
      {/* CORREZIONE: Usiamo aspect-[3/2] per forzare la proporzione rettangolare orizzontale 
        tipica di una vera busta da lettera, indipendentemente dallo schermo!
      */}
      <div 
      ...
      >
        ...
        {/* La Lettera (Animazione corretta: scivola su del 40% per fare capolino) */}
        <motion.div 
        ...
        >
           ...
           <p className="text-stone-500 uppercase tracking-widest text-[10px] md:text-xs">L'Invito</p>
        </motion.div>
